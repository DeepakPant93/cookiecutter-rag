...
bake-and-test-deploy: ## For quick publishing to cookiecutter-rag-example to test GH Actions
	...
	@cookiecutter --no-input . --overwrite-if-exists \
		...
		github_author_handle=DeepakPant93 \
		project_name=cookiecutter-rag-example \
		project_slug=cookiecutter_rag_example
	@cd cookiecutter-rag-example; poetry install && \
		git init -b main && \
		git add . && \
...
test: ## Test the code with pytest.
	...
	@poetry run pytest --cov --cov-config=pyproject.toml
